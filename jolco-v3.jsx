...
export default function JOLCOv3() {
  ...
  const R = useMemo(() => {
    ...
    */
    ...
    const equityCF_noTax = [-equity];
    const years = [];
    let outstandingTotal = VP;
    let outstandingDebt = debt;
    let outstandingEquity = equity;
    let cumulativeEquityCF = -equity;
    let totalStream1 = 0, totalStream2 = 0, totalStream3 = 0;
    ...
    for (let yr = 1; yr <= effectiveExerciseYear; yr++) {
      ...
      const spcTaxablePL = totalHire - dep.total - bankInterest;
      ...
      years.push({
        yr, fixedHire, variableHire, totalHire,
    ...
    const treasTerminal = equity * Math.pow(1 + treasuryYield / 100, effectiveExerciseYear);
    const treasProfit = treasTerminal - equity;
    const jolcoProfit = equityCF.reduce((a, b) => a + b, 0);
    const spread = blendedIRR != null ? blendedIRR - treasuryYield / 100 : null;

    return { VP, debt, equity, equityCF, equityCF_noTax, years, depr, blendedIRR, equityIRR, treasTerminal, treasProfit, jolcoProfit, spread, totalStream1, totalStream2, totalStream3, monthlyFixed, allInRate, poPriceMil };
  }, [vesselPrice, debtPct, amortYrs, sofrRate, spreadBps, commission, taxRate, usefulLife, specialDeprPct, treasuryYield, flagId, effectiveExerciseYear, poSchedule]);
      ...
      <div style={{ background: "linear-gradient(135deg, #1a1b26, #24283b)", borderBottom: "1px solid #292e42", padding: "20px 28px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#565f89" }}>BB Owner Equity Returns · MOF Depreciation · Tax Shield Analysis</div>
          </div>
        </div>
      </div>
        ...
        {tab === "deal" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            <div style={C}>
              ...
              <Inp label="Debt / Equity Split (Debt %)" value={debtPct} onChange={setDebtPct} unit="%" help="Typically 70-75% debt" min={0} max={95} />
              <Inp label="Commission" value={commission} onChange={setCommission} unit="%" help="Total brokerage commission" step={0.5} />
              <div style={{ padding: 10, borderRadius: 6, background: "#1e2030", border: "1px solid #292e42", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <div><div style={{ fontSize: 9, color: "#565f89" }}>EQUITY IN</div><div style={{ fontSize: 16, fontWeight: 700, color: "#7aa2f7", fontFamily: F }}>${$d(R.equity / 1e6, 1)}M</div></div>
                <div><div style={{ fontSize: 9, color: "#565f89" }}>DEBT</div><div style={{ fontSize: 16, fontWeight: 700, color: "#e0af68", fontFamily: F }}>${$d(R.debt / 1e6, 1)}M</div></div>
              </div>
            </div>
            ...
            <div style={C}>
              ...
              <div style={{ marginTop: 10, borderTop: "1px solid #292e42", paddingTop: 10 }}>
                <Inp label="Effective Tax Rate" value={taxRate} onChange={setTaxRate} unit="%" help="~30.62% incl. defense surtax" step={0.01} />
                <Inp label="US Treasury Yield" value={treasuryYield} onChange={setTreasuryYield} unit="%" step={0.01} />
                <Slider label="Special Depreciation (Yr1)" value={specialDeprPct} onChange={setSpecialDeprPct} min={0} max={32} step={1} unit="%" help="MLIT advanced vessels: JP 20-32%, Foreign 18-30%" />
              </div>
            </div>
            ...
            <div style={{ gridColumn: "1 / -1", ...C, background: "linear-gradient(135deg, #1a1b26, #1e2030)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
                {[
                  { l: "Equity In", v: `$${$d(R.equity / 1e6, 1)}M`, c: "#7aa2f7" },
        ...
        {tab === "depr" && (
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
            <div style={C}>
              ...
              <Slider label="Special Depreciation Rate" value={specialDeprPct} onChange={setSpecialDeprPct} min={0} max={32} step={1} unit="%" help="Slide to see how Yr1 bonus changes the profile" />
          ...
          <div>
            ...
            <div style={C}>
              ...
              {(() => {
                ...
                const rows = [
                  { label: "Equity Invested", val: -eq, color: "#f7768e", sub: "Cash out at Year 0", isNeg: true },
                  { label: "divider" },
                  { label: "Principal Returned via Hire", val: principalBack, color: "#7aa2f7", sub: `$${$d(eq/1e6,2)}M invested → $${$d(principalBack/1e6,2)}M returned over ${effectiveExerciseYear} yrs` },
                  { label: "① Interest on Equity Balance", val: s1, color: "#9ece6a", sub: `Charterer pays SOFR+${spreadBps}bps on full balance; bank only takes interest on debt portion` },
                  ...
                  { label: "NET PROFIT", val: profit, color: profit >= 0 ? "#9ece6a" : "#f7768e", sub: "= ① + ② + ③ (return ON capital, above getting your money back)", isBold: true },
                ];
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: 16, padding: 14, borderRadius: 8, background: "#16161e", border: "1px solid #292e42" }}>
                      ...
                      <div style={{ fontFamily: F, fontSize: 13, color: "#a9b1d6", lineHeight: 2.2, textAlign: "center" }}>
                        <span style={{ color: "#f7768e" }}>-${$d(eq/1e6,2)}M</span>
                        <span style={{ color: "#565f89" }}> equity in</span>
                        <br />
                        ...
                        <span style={{ color: "#565f89" }}> ① interest earned</span>
                        <br />
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 12, fontSize: 10, flexWrap: "wrap" }}>
              <span style={{ color: "#9ece6a" }}>● ① Hire Spread</span>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                ...
                <tbody>
                  <tr style={{ borderBottom: "1px solid #1e2030", background: "#1e1e2e" }}>
                    ...
                    <td colSpan={3} style={{ padding: "5px 8px", textAlign: "center", color: "#565f89", fontSize: 10 }}>Equity Investment</td>
                    <td style={{ padding: "5px 8px", textAlign: "right", fontFamily: F, color: "#f7768e", fontWeight: 600 }}>-${$(R.equity)}</td>
                    <td style={{ padding: "5px 8px", textAlign: "right", fontFamily: F, color: "#f7768e" }}>-${$(R.equity)}</td>
                  </tr>
                  {R.years.map((y, i) => {
                    ...
                    <tr style={{ borderBottom: isExpanded ? "none" : "1px solid #1e2030", background: y.yr === effectiveExerciseYear ? "rgba(122,162,247,0.04)" : "transparent" }}>
                      ...
                      <td onClick={() => setExpandedYear(isExpanded ? null : y.yr)}
                        ...
                        ${$(y.hireSpread)} <span style={{ fontSize: 8, color: "#565f89" }}>{isExpanded ? "▲" : "▼"}</span>
                      </td>
                          ...
                          <div style={{ margin: "0 8px 8px", padding: 12, borderRadius: 6, background: "#16161e", border: "1px solid #3b4261" }}>
                            ...
                            <div style={{ fontFamily: F, fontSize: 11, lineHeight: 2, color: "#a9b1d6" }}>
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between" }}>
                                ...
                                <span style={{ color: "#9ece6a", fontWeight: 700 }}>${$(y.equityPrincipalReturn + y.equityInterestIncome)}</span>
                              </div>
            ...
            <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              {[
                ...
                { l: "MoIC", v: R.equity > 0 ? $d((R.equity + R.jolcoProfit) / R.equity, 2) + "x" : "—", s: "Total returned / equity", c: "#7aa2f7" },
              ].map((x, i) => (
          ...
          <div>
            <div style={{ ...C, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div style={{ padding: 18, borderRadius: 10, background: "#16161e", border: "1px solid #292e42" }}>
                ...
                {[
                  { l: "Equity In", v: `$${$d(R.equity / 1e6, 2)}M`, c: "#7aa2f7" },
                  ...
                  { l: "Total Profit", v: `$${$d(R.jolcoProfit / 1e6, 2)}M`, c: R.jolcoProfit >= 0 ? "#9ece6a" : "#f7768e" },
                  { l: "MoIC", v: $d((R.equity + R.jolcoProfit) / R.equity, 2) + "x" },
                ].map((r, i) => (
